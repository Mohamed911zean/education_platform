...
import { ENROLLED, GRADES, SUBJECTS } from "../data";
  ...
  return (
    <div className="flex min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)]" dir="rtl">
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        <DashboardNavbar breadcrumb={[{ label: "الرئيسية", href: "/dashboard" }, { label: "الكورسات", href: "/courses" }, { label: "كورسات مشترك فيها" }]} />

        <main className="flex-1 p-4 pb-24 md:p-6 lg:p-8 overflow-y-auto space-y-8 max-w-7xl mx-auto w-full">
          
          <div className="flex justify-between items-end">
            <div>
              <h1 className="text-2xl font-bold font-cairo">كورسات مشترك فيها</h1>
              <p className="text-sm text-[var(--text-muted)] font-cairo mt-1">تتبع تقدمك في كورساتك الحالية</p>
            </div>
          </div>
          ...
          <section>
            {filteredCourses.length === 0 ? (
              <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-default)] text-center py-20 shadow-sm animate-fade-up">
                ...
                <div className="text-lg font-bold font-cairo text-[var(--text-primary)] mt-4">لا توجد كورسات مشتركة</div>
                <div className="text-sm text-[var(--text-muted)] font-cairo mt-1">حاول تغيير خيارات البحث</div>
              </div>
            ) : (
